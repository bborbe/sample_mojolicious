...
helper db => sub {
    if($dbh){
      ...
      #$dbh = DBI->connect('DBI:mysql:database=yourdb;host=localhost','dbuser','dbpassword') or die $DBI::errstr;
      $dbh = DBI->connect('DBI:Pg:database=test;host=localhost','dbuser','dbpassword') or die $DBI::errstr;
        return $dbh;
    }
};
...
get '/data/:id' => sub {
    ...
    my $SQL = "select data, data2 from my_table where id = '$id'";
    my $cursor = $self->db->prepare($SQL);
    $cursor->execute;
    ...
    return $self->render(json => {data => $data[0], data2 => $data[1]});
